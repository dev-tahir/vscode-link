...
let chatHistory: { role: string; content: string; timestamp: string }[] = [];
let capturedMessages: any[] = [];
let outputChannel: vscode.OutputChannel;
let clipboardWatcher: NodeJS.Timeout | null = null;
let lastClipboard = '';
let fileWatchers: vscode.FileSystemWatcher[] = [];
let chatStorageContent: any[] = [];
...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('remoteChatControl.sendMessage', () => promptAndSend()),
        vscode.commands.registerCommand('remoteChatControl.readMessages', showHistory),
        vscode.commands.registerCommand('remoteChatControl.startServer', startServer),
        vscode.commands.registerCommand('remoteChatControl.scanChatStorage', scanChatStorage),
        vscode.commands.registerCommand('remoteChatControl.startClipboardWatch', startClipboardWatch)
    );
...
async function sendToChat(message: string, model?: string, sessionMode?: string, sessionId?: string): Promise<{ result: string; note?: string }> {
...
function looksLikeChatContent(text: string): boolean {
    const indicators = [
        /copilot/i,
        /```[\s\S]*```/,  // Code blocks
...
function startNetworkIntercept() {
    ...
    (http as any).request = function(...args: any[]) {
        const req = originalHttpRequest.apply(http, args as any);
    ...
    (https as any).request = function(...args: any[]) {
        const req = originalHttpsRequest.apply(https, args as any);
...
function interceptRequest(req: http.ClientRequest, args: any[], protocol: string) {
    try {
...
async function findMostRecentWorkspaceFolder(basePath: string): Promise<string | null> {
    try {
        const entries = fs.readdirSync(basePath, { withFileTypes: true });
        let mostRecentFolder: { path: string; mtime: number } | null = null;
...
async function scanDirectoryForChat(dirPath: string, depth: number) {
    if (depth > 6) return; // Increased depth to scan more files
...
async function readChatSessionFile(filePath: string) {
    try {
...
function extractChatSessionMessages(data: any): Array<{role: string, text: string}> {
...
async function readVscdbFile(filePath: string) {
    try {
...
function extractMessagesFromBinary(content: string): string[] {
...
async function readJsonFileForChat(filePath: string) {
    try {
        const stats = fs.statSync(filePath);
        if (stats.size > 10 * 1024 * 1024 || stats.size < 10) return; // Skip very large or tiny files
...
function extractMessagesFromJson(data: any, prefix = ''): Array<{role: string, text: string}> {
    const messages: Array<{role: string, text: string}> = [];
...
function watchChatStorageFiles(context: vscode.ExtensionContext) {
...
async function getLatestReply(): Promise<{
    success: boolean;
    userMessage?: string;
    ...
    try {
        ...
        const userMessage = lastRequest.message?.text || 
            (lastRequest.message?.parts?.find((p: any) => p.kind === 'text')?.text) || '';
...
async function waitForReplyToMessage(targetMessage: string, maxWaitSeconds: number = 60): Promise<{
    success: boolean;
    userMessage?: string;
    assistantReply?: string;
        ...
        try {
            ...
            for (const folder of folders) {
                ...
                for (const file of files) {
                    ...
                    if (stats.mtime.getTime() < fiveMinutesAgo) continue;
                    
                    const content = fs.readFileSync(filePath, 'utf-8');
                    const data = JSON.parse(content);
                    
                    if (!data.requests || data.requests.length === 0) continue;
...
function showHistory() {
    ...
    for (const [type, messages] of Object.entries(byType)) {
        ...
        messages.slice(-5).forEach((msg, i) => {
            ...
            if (msg.content) channel.appendLine(`Content: ${msg.content.substring(0, 500)}`);
            if (msg.url) channel.appendLine(`URL: ${msg.url}`);
            if (msg.response) channel.appendLine(`Response: ${msg.response.substring(0, 500)}`);
            if (msg.path) channel.appendLine(`Path: ${msg.path}`);
            channel.appendLine('');
        });
    }
    ...
    if (chatSessions.length === 0) {
        ...
        chatSessions.forEach((session, i) => {
            ...
            if (session.userMessages && session.userMessages.length > 0) {
                ...
                session.userMessages.forEach((msg: any, mi: number) => {
            ...
            if (session.messages && session.messages.length > 0) {
                ...
                session.messages.forEach((msg: any, mi: number) => {
        ...
        if (url === '/api/command-action' && req.method === 'POST') {
            ...
            req.on('end', async () => {
                try {
                    ...
                    exec(`powershell -Command "${activateScript}"`, async (activateError: any) => {
                        if (activateError) {
                        ...
                        if (action === 'approve') {
                            ...
                            exec(`powershell -Command "${psScript}"`, (err: any) => {
                                if (err) {
